...
export function DocumentoPreview({ dados, onFechar }: Props) {
  ...
  const nomeMarca = branding?.nome_exibicao || branding?.nome_fantasia || branding?.razao_social || dados.contabilidade.escritorio || 'Destrava Crédito';
  const corPrimaria = branding?.cor_primaria || '#1B3A6B';
  const cidade = dados.cidade ?? ([branding?.cidade, branding?.uf].filter(Boolean).join(' - ') || 'Brasília - DF');
  const isDeclaracao = dados.tipo === 'declaracao';
  const rodape = [branding?.rodape_linha_1, branding?.rodape_linha_2, branding?.rodape_linha_3].filter(Boolean).join('  |  ')
    || [nomeMarca, branding?.email, branding?.telefone || branding?.whatsapp].filter(Boolean).join('  |  ');
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm overflow-y-auto py-6 px-4">
      ...
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-0 print:p-0">
          ...
          <div className="bg-[#1B3A6B] px-8 pt-6 pb-0" style={{ backgroundColor: corPrimaria }}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                {branding?.logo_data_uri && <img src={branding.logo_data_uri} alt={nomeMarca} className="max-h-12 max-w-40 object-contain bg-white rounded px-2 py-1" />}
                <div>
                  <p className="text-white text-lg font-bold tracking-wide">{nomeMarca.toUpperCase()}</p>
                  <p className="text-blue-100 text-xs mt-0.5">Documento contábil</p>
                </div>
              </div>
            ...
            {/* Faixa de empresa */}
            <div className="mt-4 bg-[#29559B] -mx-8 px-8 py-2" style={{ backgroundColor: corPrimaria }}>
          ...
          <div className="px-8 py-4 space-y-3">
            ...
            <div className="text-center space-y-1">
              <h2 className="text-[#1B3A6B] text-base font-bold tracking-tight" style={{ color: corPrimaria }}>{tituloDoc}</h2>
              <div className="w-24 h-0.5 bg-[#1B3A6B] mx-auto" style={{ backgroundColor: corPrimaria }} />
            ...
            <div className="rounded-lg overflow-hidden border border-gray-200">
              {/* Cabeçalho da tabela */}
              <div className="bg-[#1B3A6B] grid grid-cols-2 px-4 py-1.5" style={{ backgroundColor: corPrimaria }}>
              ...
              {/* Total */}
              <div className="bg-[#1B3A6B] grid grid-cols-2 px-4 py-2" style={{ backgroundColor: corPrimaria }}>
          ...
          {/* Rodapé */}
          <div className="bg-[#1B3A6B] px-8 py-2.5 border-t-2 border-[#b4a05a]" style={{ backgroundColor: corPrimaria }}>
            <p className="text-blue-200 text-[9px] text-center">
              {rodape || `Documento gerado eletronicamente — ${nomeMarca}`}
            </p>
          </div>
        </div>
      </div>
