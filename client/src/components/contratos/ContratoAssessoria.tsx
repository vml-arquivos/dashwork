...
import { getCachedAccountBranding } from '../../lib/accountBranding';
...
export function ContratoAssessoria({ dados, documentosAnexos = [], onClose, onGerarPdf, loadingPdf }: Props) {
  ...
  const assinantesContratante = d.modo_assinatura_contratante === 'socios' && sociosAssinantes.length > 0
    ...
    : representantePrincipalContratante.nome
      ...
      : [];
  const branding = getCachedAccountBranding();
  const contratadaNome = branding?.razao_social || branding?.nome_exibicao || branding?.nome_fantasia || 'DESTRAVA CREDITO LTDA';
  const contratadaMarca = branding?.nome_exibicao || branding?.nome_fantasia || branding?.razao_social || 'Destrava Crédito';
  const contratadaCnpj = branding?.cnpj || '35.427.182/0001-66';
  const contratadaEndereco = [branding?.endereco, branding?.cidade, branding?.uf, branding?.cep].filter(Boolean).join(' — ')
    || 'QD QND 25, LOTE 40, Taguatinga Norte – Brasília - DF, Cep: 72.120-250';
  const representanteContratada = branding?.signatario_nome || 'FERNANDO ELI OLIVEIRA MARQUES';
  const cargoContratada = branding?.signatario_cargo || 'Sócio Administrador';
  const cpfContratada = branding?.signatario_cpf || '718.517.041-91';
  const rodapeConta = [branding?.rodape_linha_1, branding?.rodape_linha_2, branding?.rodape_linha_3].filter(Boolean) as string[];
      ...
      <div className="flex flex-1 overflow-hidden print:block print:overflow-visible">
        ...
        <main className="flex-1 overflow-auto p-6 print:p-0 print:overflow-visible">
          <div style={S.page}>
            ...
            <div style={S.header}>
              <div style={S.logoBox}>
                {branding?.logo_data_uri ? (
                  <img src={branding.logo_data_uri} alt={contratadaMarca} style={{ maxWidth: '145px', maxHeight: '48px', objectFit: 'contain' }} />
                ) : (
                  <>
                    <span style={S.logoText}>{contratadaMarca}</span>
                    <span style={S.logoSub}>ASSESSORIA EMPRESARIAL</span>
                  </>
                )}
              </div>
              <div style={S.headerRight}>
                <div>{contratadaNome}</div>
                {contratadaCnpj && <div>CNPJ: {contratadaCnpj}</div>}
                {contratadaEndereco && <div>{contratadaEndereco}</div>}
              </div>
            </div>
            ...
            <p style={S.p}>
              <strong>CONTRATADA:</strong> denominada {contratadaNome}, com sede na {contratadaEndereco}, inscrita no CNPJ n° {contratadaCnpj}, devidamente representada por {representanteContratada}, {cargoContratada}{cpfContratada ? `, CPF n° ${cpfContratada}` : ''}.
            </p>
            ...
            <p style={S.p}>
              <strong>5.5. Deferimento Interno e Abertura de Conta:</strong> Mediante parecer favorável da Diretoria Técnica da {contratadaMarca.toUpperCase()}, os documentos serão processados e encaminhados para os trâmites de abertura de conta corrente de pessoa jurídica junto às instituições parceiras.
            </p>
            <p style={S.p}>
            ...
            <div style={S.partyGroup}>
              <p style={S.partyHeading}>CONTRATADA — {contratadaNome}{contratadaCnpj ? ` · CNPJ: ${contratadaCnpj}` : ''}</p>
              <div style={S.indRow}>
                <div style={S.indCard}>
                  <div style={S.indSpace} />
                  <div style={S.sigLine} />
                  <p style={S.sigName}>{representanteContratada}</p>
                  <p style={S.sigSub}>{cargoContratada}</p>
                </div>
              </div>
            </div>
            ...
            <div style={S.footer}>
              {(rodapeConta.length ? rodapeConta : [contratadaNome, contratadaEndereco, [branding?.email, branding?.telefone || branding?.whatsapp].filter(Boolean).join(' · ')].filter(Boolean) as string[]).map((linha, i) => (
                <p key={`${linha}-${i}`} style={{ marginTop: i ? '4px' : 0 }}>{linha}</p>
              ))}
              <p style={{ marginTop: '4px', fontSize: '7pt' }}>
                Documento gerado eletronicamente por {contratadaMarca} em {new Date().toLocaleDateString('pt-BR')}.
