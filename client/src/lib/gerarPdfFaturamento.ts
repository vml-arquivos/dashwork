...
import { getCachedAccountBranding, type AccountBrandingClient } from '@/lib/accountBranding';

// ─── Paleta de cores (consistente com a marca Destrava) ───────────────────────
let AZUL_ESCURO: [number, number, number] = [27, 58, 107];   // #1B3A6B
let AZUL_MEDIO: [number, number, number] = [41, 85, 155];    // #29559B
let AZUL_CLARO: [number, number, number] = [227, 235, 248];  // #E3EBF8
...
let BRANDING_ATUAL: AccountBrandingClient | null = null;
function hexRgb(value: string | undefined, fallback: [number,number,number]): [number,number,number] { const m=/^#([0-9a-fA-F]{6})$/.exec(String(value||'')); if(!m)return fallback; const n=parseInt(m[1],16); return [(n>>16)&255,(n>>8)&255,n&255]; }
function aplicarBranding(){ BRANDING_ATUAL=getCachedAccountBranding(); if(BRANDING_ATUAL?.cor_primaria){ AZUL_ESCURO=hexRgb(BRANDING_ATUAL.cor_primaria,[27,58,107]); AZUL_MEDIO=AZUL_ESCURO; AZUL_CLARO=[Math.min(255,AZUL_ESCURO[0]+210),Math.min(255,AZUL_ESCURO[1]+195),Math.min(255,AZUL_ESCURO[2]+150)]; } }
// ─── Tipografia / helpers ─────────────────────────────────────────────────────
...
function desenharCabecalho(
  ...
  if (BRANDING_ATUAL?.logo_data_uri) { try { doc.addImage(BRANDING_ATUAL.logo_data_uri, 'PNG', ML, 7, 40, 10); } catch {} }
  doc.text((BRANDING_ATUAL?.nome_exibicao || contabilidade.escritorio).toUpperCase(), BRANDING_ATUAL?.logo_data_uri ? ML + 46 : ML, 14);

  // Subtítulo institucional
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text(BRANDING_ATUAL?.razao_social || 'Escritório de Contabilidade — Serviços Contábeis e Fiscais', ML, 20);
...
function desenharRodape(doc: jsPDF): void {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    [BRANDING_ATUAL?.rodape_linha_1, BRANDING_ATUAL?.rodape_linha_2, BRANDING_ATUAL?.rodape_linha_3].filter(Boolean).join('  |  ') || `Documento gerado eletronicamente — ${BRANDING_ATUAL?.nome_exibicao || 'Destrava Crédito'}  |  ${BRANDING_ATUAL?.email || 'destravacreditooficial@gmail.com'}  |  ${BRANDING_ATUAL?.telefone || BRANDING_ATUAL?.whatsapp || '(61) 3526-8355'}`,
    W / 2,
    H - 5,
    { align: 'center' },
  );
