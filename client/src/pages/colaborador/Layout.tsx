...
export default function Layout({
  ...
  const [accountBranding, setAccountBranding] = useState<AccountBrandingClient | null>(() => getCachedAccountBranding());
  useEffect(() => {
    let active = true;
    if (colaborador?.id) void loadAccountBranding().then((brand) => { if (active && brand) setAccountBranding(brand); });
    return () => { active = false; };
    ...
    <nav className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-4">
        ...
        <div className="min-w-0">
          <div className="truncate text-xs font-black text-slate-800 leading-tight">
            {accountName}
          </div>
