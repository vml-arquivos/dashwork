...
export default function Orcamentos() {
  ...
  const nomePrestadoraConta = brandingConta?.nome_exibicao || brandingConta?.nome_fantasia || brandingConta?.razao_social || colaborador?.conta_nome || "Minha Empresa";
  ...
  useEffect(() => {
    if (isContaLegada || selecionado || !brandingConta) return;
    setForm((f: any) => ({
      ...f,
      marca: "destrava",
      assinaturas: Array.isArray(f.assinaturas)
        ? f.assinaturas.map((a: any, i: number) => i === 0 && a?.tipo === "contratada" ? ({
            ...a,
            nome: brandingConta.razao_social || nomePrestadoraConta,
            cargo: brandingConta.signatario_cargo || "Contratada",
            documento: brandingConta.cnpj ? `CNPJ ${brandingConta.cnpj}` : "",
          }) : a)
        : f.assinaturas,
    }));
  }, [brandingConta, isContaLegada, selecionado, nomePrestadoraConta]);
              ...
              <div className="p-4">
                      ...
                      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                        ...
                        <label className="block">
                          ...
                          {isContaLegada ? (
                            ...
                            <div className="flex h-11 w-full items-center rounded-2xl border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-slate-700">
                              {nomePrestadoraConta}
                            </div>
                          )}
                        </label>
                      ...
                      <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <div className="space-y-3 text-sm">
                          ...
                          <div>
                            ...
                            <p className="font-bold text-slate-900">
                              {isContaLegada
                                ? (marca === "permupay" ? "PermuPay" : marca === "aragao" ? "Aragão Serviços" : "Destrava Crédito")
                                : nomePrestadoraConta}
                            </p>
                          </div>
                          <div>
                ...
                {aba === "preview" && (
                  <div className="mx-auto max-w-4xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
                    <div
                      className={`mb-6 flex items-center justify-between border-b-4 pb-4 ${isContaLegada ? (marca === "permupay" ? "border-blue-600" : marca === "aragao" ? "border-amber-600" : "border-[#1B3A8C]") : "border-slate-300"}`}
                      style={!isContaLegada ? { borderBottomColor: brandingConta?.cor_primaria || "#1B3A8C" } : undefined}
                    >
                      {!isContaLegada ? (
                        brandingConta?.logo_data_uri
                          ? <img alt={`Logotipo ${nomePrestadoraConta}`} src={brandingConta.logo_data_uri} className="h-14 max-w-[240px] object-contain" />
                          : <div className="text-xl font-black" style={{ color: brandingConta?.cor_primaria || "#1B3A8C" }}>{nomePrestadoraConta}</div>
                      ) : (
                        <img
                          alt={`Logotipo ${marca === "permupay" ? "Permupay" : marca === "aragao" ? "Aragão Serviços" : "Destrava Crédito"}`}
                          src={marca === "permupay" ? "/logo-permupay.png" : marca === "aragao" ? "/logo-aragao-servicos.svg" : "/destrava-logo.svg"}
                          onError={(e) => { (e.currentTarget as HTMLImageElement).src = "/destrava-logo.svg"; }}
