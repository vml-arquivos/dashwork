...
async function gerarHtmlOrcamento(orcamento: Row): Promise<string> {
  const branding = await getAccountBranding().catch(() => null);
  const marca = normalizeMarca(orcamento.marca);
  const cor = branding?.cor_primaria || (marca === "permupay" ? "#075FAE" : marca === "aragao" ? "#7A4328" : "#173A79");
  const corEscura = branding?.cor_primaria || (marca === "permupay" ? "#064D8D" : marca === "aragao" ? "#5F321E" : "#102E63");
  ...
  const assinaturasHtml = assinaturas.length
    ...
    : `<div class="sig-box"><div class="sig-line"></div><strong>${escapeHtml(branding?.nome_exibicao || marcaNome(marca))}</strong><span>Contratada</span></div>
...
async function gerarPdfOrcamentoPuppeteer(orcamento: Row): Promise<Buffer> {
  return generateBrandedPdfBuffer(await gerarHtmlOrcamento(orcamento), {
...
async function gerarPdfOrcamentoFallback(orcamento: Row, motivo: string): Promise<Buffer> {
  ...
  draw(branding?.nome_exibicao || marcaNome(orcamento.marca), { size: 18, bold: true, gap: 8, maxChars: 50, color: rgb(0.06, 0.18, 0.38) });
...
async function gerarPdfOrcamentoUltimoRecurso(orcamento: Row): Promise<Buffer> {
  ...
  const branding = await getAccountBranding().catch(() => null);
  linha(`Entre em contato com ${branding?.nome_exibicao || 'a empresa emissora'} para a via completa.`, 9);
  return Buffer.from(await doc.save());
}
