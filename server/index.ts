...
async function startServer() {
      ...
      const result = await systemPool.query(
        `SELECT c.id, c.email, c.nome, c.cargo, c.senha_hash, c.ativo, c.chatwoot_agente_id,
                ...
                COALESCE(c.perfil, CASE
                  ...
                  ELSE 'agente'
                END) AS perfil,
                COALESCE(c.pode_atender_leads, CASE WHEN LOWER(COALESCE(c.cargo, '')) IN ('captador externo', 'estagiário', 'estagiario') THEN FALSE ELSE TRUE END) AS pode_atender_leads,
                COALESCE(c.pode_ver_todos_leads, CASE
                  WHEN LOWER(COALESCE(c.cargo, '')) IN ('administrador', 'admin', 'diretor', 'gerente comercial', 'gerente', 'gestor') THEN TRUE
                  ELSE FALSE
                END) AS pode_ver_todos_leads
           FROM colaboradores c
           JOIN contas_plataforma cp ON cp.id = c.conta_id
          WHERE c.email = $1`,
        [email.trim().toLowerCase()]
      );
      const user = result.rows[0];
      if (!user || !user.ativo || user.conta_status !== "ativo") {
  ...
  app.get("/api/configuracao-funcoes/me", auth, async (req: Request, res: Response) => {
    try {
      ...
      const conta = await systemPool.query(`SELECT perfil_base, modulos_ativos FROM contas_plataforma WHERE id=$1`, [colaborador?.conta_id]);
      res.json({
        accountProfile: conta.rows[0]?.perfil_base || null,
        accountModules: conta.rows[0]?.modulos_ativos || [],
  ...
  app.get("/api/empresas/:id/proposta-bancaria", auth, async (req: Request, res: Response) => {
    try {
      ...
      const brandingProposta = await getAccountBranding().catch(() => null);
      const marcaProposta = brandingProposta?.nome_exibicao || brandingProposta?.nome_fantasia || brandingProposta?.razao_social || "Destrava Crédito";
  ...
  app.get("/api/empresas/:id/proposta-bancaria/pdf", auth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const brandingProposta = await getAccountBranding().catch(() => null);
      const marcaProposta = brandingProposta?.nome_exibicao || brandingProposta?.nome_fantasia || brandingProposta?.razao_social || "Destrava Crédito";
      const proposta = calcularPropostaBancaria({ empresa, socios, documentos, simulacoes, orcamentos: [], contratos, historico: [], marca_sistema: marcaProposta });
      ...
      const html = gerarHtmlTimbrado(`
        <div style="font-family: Arial, sans-serif; color: #1e293b;">
          ...
          <table style="width:100%; border-collapse: collapse; margin-bottom: 16px; font-size: 12px;">
            ...
            <tr><td style="padding:6px 12px; border-bottom:1px solid #f1f5f9; color:#64748b;">${proposta.score_label}</td><td style="padding:6px 12px; border-bottom:1px solid #f1f5f9; font-weight:bold;">${proposta.score_destrava}/100</td>
  ...
  app.get("/api/empresas/:id/relatorio-tecnico", auth, async (req: Request, res: Response) => {
    try {
      ...
      const brandingRelatorio = await getAccountBranding().catch(() => null);
      const marcaRelatorioConta = brandingRelatorio?.nome_exibicao || brandingRelatorio?.nome_fantasia || brandingRelatorio?.razao_social || "Destrava Crédito";
      const resultado = gerarRelatorioTecnico({
        ...
        contratos,
        historico,
        responsavel_nome: colaborador?.nome || colaborador?.email || `Sistema ${marcaRelatorioConta}`,
        marca_sistema: marcaRelatorioConta,
      });
      ...
      const html = gerarHtmlTimbrado(`
        ...
        <div style="background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: white; padding: 32px 28px; border-radius: 12px; margin-bottom: 24px;">
          ...
          <div style="margin-top:16px; display:flex; gap:24px; flex-wrap:wrap;">
            <div><p style="margin:0; font-size:10px; opacity:0.7;">${rel.score_label}</p><p style="margin:0; font-size:24px; font-weight:900;">${rel.analise_credito.score_destrava}/100</p></div>
        ...
        <div class="rt-section">
          ...
          <table class="rt-table">
            <tr><th>${rel.score_label}</th><td><strong>${rel.analise_credito.score_destrava}/100</strong></td><th>Nível de Risco</th><td style="color:${riscoCor}; font-weight:700;">${rel.analise_credito.nivel_risco.toUpperCase()}</td></tr>
        ...
        <div style="margin-top:12px; font-size:10px; color:#94a3b8; text-align:center;">
          Relatório gerado em ${new Date(rel.gerado_em).toLocaleString('pt-BR')} por ${rel.responsavel_analise} &mdash; ${rel.marca_sistema}
        </div>
  ...
  app.get("/api/empresas/:id/relatorio-tecnico/zip", auth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const htmlSimples = gerarHtmlTimbrado(`
        ...
        <div style="margin-top:12px; font-size:10px; color:#94a3b8; text-align:center;">Gerado em ${new Date(rel.gerado_em).toLocaleString('pt-BR')} por ${rel.responsavel_analise} &mdash; ${rel.marca_sistema}</div>
...
<div style="border:1px solid #d1d5db;border-radius:8px;padding:10px 12px;margin:0 0 18px 0;background:#f9fafb;font-size:11px;line-height:1.45;">
  ...
  async function gerarHtmlContrato(payload: any): Promise<string> {
    ...
    const contratadaNormalizada = payload.contratada || await contratadaContaAtualNormalizada();
    const contratada = contratadaParaFormatoLegado(contratadaNormalizada);
    const marcaContratada = contratadaNormalizada?.nome_exibicao || contratadaNormalizada?.nome_fantasia || contratadaNormalizada?.razao_social || contratada.razao_social || 'Contratada';
    const emailComunicacaoContratada = contratadaNormalizada?.email || (await getAccountBranding().catch(() => null))?.signatario_email || (await getAccountBranding().catch(() => null))?.email || '';
...
<p class="clause"><strong>Cláusula 3</strong> - Este contrato terá vigência de <strong>${vigenciaMeses} (${vigenciaExtenso}) meses</strong> a contar da data de sua assinatura, sendo automaticamente renovado por igual período, caso não haja manifestação contrária de qualquer das partes, comunicada com no mínimo 30 (trinta) dias de antecedência do vencimento, por meio de e-mail enviado ao endereço: ${escapeHtmlContrato(emailComunicacaoContratada || 'canal eletrônico informado pela CONTRATADA')}.</p>
...
<p class="clause"><strong>5.5. Deferimento Interno e Abertura de Conta:</strong> Mediante parecer favorável da Diretoria Técnica da ${escapeHtmlContrato(String(marcaContratada).toUpperCase())}, os documentos serão processados e encaminhados para os trâmites de abertura de conta corrente de pessoa jurídica junto às instituições parceiras.</p>
...
<head>
  <meta charset="UTF-8"/>
  <title>${escapeHtmlContrato(marcaContratada)} — CONTRATO DE ANÁLISE DOCUMENTAL</title>
  <style>
    ${getDocumentStyles()}
...
<head>
  <meta charset="UTF-8"/>
  <title>${escapeHtmlContrato(marcaContratada)} — CONTRATO DE ASSESSORIA PF</title>
  <style>
    ${getDocumentStyles()}
    body { padding: 0; background: #fff; }
    .contract-content { width: 100%; }
...
<body>
  ...
  function gerarHtmlPrevisaoFaturamento(payload: {
    ...
    const brandingConta = getCachedAccountBranding();
    const localEmissao = [brandingConta?.cidade, brandingConta?.uf].filter(Boolean).join(' - ') || 'Brasília - DF';
    const headerConta = getHtmlHeaderEmbutido();
    const footerConta = getHtmlFooterEmbutido();
...
.contract-footer-final { margin-top:24px; padding-top:8px; border-top:1px solid #d1d5db; text-align:center; font-size:7.5pt; color:#64748b; line-height:1.45; page-break-inside:avoid; }
  </style>
</head>
<body>
  ${headerConta}
  ...
  <div class="city-date">${esc(localEmissao)}, ${esc(dataEmissao)}.</div>
  ...
  function gerarHtmlDeclaracaoAnual(payload: {
    ...
    const brandingConta = getCachedAccountBranding();
    const localEmissao = [brandingConta?.cidade, brandingConta?.uf].filter(Boolean).join(' - ') || payload.cidade || 'Brasília - DF';
    const headerConta = getHtmlHeaderEmbutido();
    const footerConta = getHtmlFooterEmbutido();
...
<body>
  ...
  <div class="city-date">${esc(localEmissao)}, ${esc(dataEmissao.toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' }))}.</div>
  <div class="signature-section">
    <div class="signature-grid">
      <div class="signature-card">
        <div class="signature-line"></div>
  ...
  function gerarHtmlSimulacao(payload: {
    ...
    const brandingConta = getCachedAccountBranding();
    const nomeMarca = brandingConta?.nome_exibicao || brandingConta?.nome_fantasia || brandingConta?.razao_social || 'Destrava Crédito';
    const razaoMarca = brandingConta?.razao_social || nomeMarca;
    const cnpjMarca = brandingConta?.cnpj || '35.427.182/0001-66';
    const localEmissao = [brandingConta?.cidade, brandingConta?.uf].filter(Boolean).join(' - ') || 'Brasília - DF';

    const body = `
<h1 class="doc-title">PROPOSTA DE CRÉDITO</h1>
<h1 class="doc-title" style="font-size:11pt; font-weight:normal; margin-bottom:24px;">Simulação de Financiamento — ${nomeMarca}</h1>
...
<p style="margin-top:16px; font-size:9pt; color:#666; font-style:italic;">
  <strong>Importante:</strong> Esta proposta é uma simulação com fins informativos. Os valores apresentados são estimativas e podem variar conforme análise de crédito, perfil do solicitante e condições da instituição financeira parceira. A aprovação do crédito está sujeita à análise documental completa pela equipe ${nomeMarca}.
</p>

<p class="city-date" style="margin-top:36px;">${localEmissao}, ${new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}.</p>

<div class="signature-block">
  <p>Consultor Responsável:</p>
  <div class="signature-line"></div>
  <p class="signature-label">${razaoMarca.toUpperCase()} — CNPJ nº ${cnpjMarca}</p>
</div>
`;
  ...
  async function contratadaContaAtualNormalizada(): Promise<any> {
    const b = await getAccountBranding().catch(() => null);
    if (!b) return contratadaDestravaNormalizada();
    const endereco = [b.endereco, [b.cidade,b.uf].filter(Boolean).join(' - '), b.cep ? `CEP: ${b.cep}` : ''].filter(Boolean).join(', ');
    return normalizarPrestadorServico({ tipo_pessoa:'pj', razao_social:b.razao_social||b.nome_exibicao, nome_fantasia:b.nome_fantasia||b.nome_exibicao, cnpj:b.cnpj||'', endereco, cidade:b.cidade||'', uf:b.uf||'', cep:b.cep||'', email:b.email||'', telefone:b.telefone||b.whatsapp||'', representante_nome:b.signatario_nome||'', representante_cpf:b.signatario_cpf||'', representante_cargo:b.signatario_cargo||'', logo_url:b.logo_data_uri||'', logo_path:b.logo_path||'', cor_primaria:b.cor_primaria, cor_secundaria:b.cor_secundaria, cidade_assinatura:b.cidade||'', uf_assinatura:b.uf||'', rodape_html:[b.rodape_linha_1,b.rodape_linha_2,b.rodape_linha_3].filter(Boolean).join('<br/>') });
  }
  function contratadaParaFormatoLegado(c:any): any {
    if (!c) return { ...CONTRATADA_DADOS };
    const x = c;
    return {
      razao_social: x.razao_social || x.nome_exibicao || x.nome_fantasia || 'CONTRATADA',
      cnpj: x.cnpj || x.documento || '',
      endereco_sede: x.endereco || '',
      representante: x.representante_nome || '',
      cargo_representante: x.representante_cargo || '',
      cpf_representante: x.representante_cpf || '',
    };
  }
...
`;
  ...
  async function gerarHtmlContratoRating(payload: any): Promise<string> {
    const { contratante, representante, contrato } = payload;
    const contratada   = payload.contratada || await contratadaContaAtualNormalizada();
    const responsavelContrato = payload.responsavel_contrato || null;
...
`;
  ...
  async function gerarHtmlContratoParceriaComercial(payload: any): Promise<string> {
    ...
    const contratada  = await contratadaContaAtualNormalizada();
    const nomeContratadaContrato = escapeHtmlContrato(contratada.nome_exibicao || contratada.razao_social || 'CONTRATADA');
...
${blocoIdentificacaoContrato(contrato)}
<p class="clause"><strong>CONTRATADA:</strong> ${qualificacaoContratada(contratada)}, doravante denominada simplesmente ${nomeContratadaContrato}.</p>
...
<h2 class="section-title">CLÁUSULA PRIMEIRA – DO OBJETO</h2>
<p class="clause"><strong>1.1.</strong> O presente contrato tem como objeto a formalização de parceria comercial entre a ${nomeContratadaContrato} e a PARCEIRA para a prospecção e captação de clientes (pessoas físicas ou jurídicas) interessados na obtenção de crédito bancário, doravante denominados CLIENTES.</p>
<p class="clause"><strong>1.2.</strong> A PARCEIRA atuará como intermediária autônoma, identificando potenciais CLIENTES, apresentando os serviços da ${nomeContratadaContrato}, recolhendo a documentação necessária para análise de crédito e prestando o suporte inicial.</p>
<p class="clause"><strong>1.3.</strong> A ${nomeContratadaContrato} será responsável por analisar a documentação fornecida, avaliar o perfil de crédito do CLIENTE, identificar a instituição financeira mais adequada e orientar o CLIENTE no processo de contratação do crédito.</p>
<h2 class="section-title">CLÁUSULA SEGUNDA – DAS OBRIGAÇÕES DA ${nomeContratadaContrato}</h2>
...
<p class="clause"><strong>3.2.</strong> Realizar o atendimento inicial, apresentar os serviços da ${nomeContratadaContrato} e esclarecer as condições gerais da prestação de serviços.</p>
<p class="clause"><strong>3.3.</strong> Recolher junto aos CLIENTES toda a documentação solicitada pela ${nomeContratadaContrato}, garantindo sua correta e completa entrega.</p>
<p class="clause"><strong>3.4.</strong> Arcar integralmente com todas as despesas decorrentes de sua atividade de prospecção e captação, incluindo, mas não se limitando a, custos com transporte, comunicação, alimentação e material de escritório. A ${nomeContratadaContrato} não realizará qualquer tipo de reembolso ou ajuda de custo.</p>
<p class="clause"><strong>3.5.</strong> Atuar com ética, boa-fé e transparência, zelando pela imagem e bom nome da ${nomeContratadaContrato} perante os CLIENTES e o mercado.</p>
<p class="clause"><strong>3.6.</strong> Encaminhar à ${nomeContratadaContrato} todas as propostas e documentos dos CLIENTES captados para a devida análise e processamento.</p>
<h2 class="section-title">CLÁUSULA QUARTA – DA REMUNERAÇÃO E FORMA DE PAGAMENTO</h2>
<p class="clause"><strong>4.1.</strong> A título de remuneração pelos serviços de assessoria prestados pela ${nomeContratadaContrato}, o CLIENTE pagará uma comissão de sucesso correspondente a 10% (dez por cento) sobre o valor total do crédito efetivamente contratado ou liberado pela instituição financeira.</p>
<p class="clause"><strong>4.2.</strong> A comissão de sucesso mencionada no item 4.1 será dividida em 2 partes entre a ${nomeContratadaContrato} e a PARCEIRA, cabendo <strong>${pctDestrava}% (${pctDestrava === 70 ? 'setenta' : String(pctDestrava)} por cento)</strong> para ${nomeContratadaContrato} e <strong>${pctParceiro}% (${pctParceiro === 30 ? 'trinta' : String(pctParceiro)} por cento)</strong> para a PARCEIRA COMERCIAL.</p>
<p class="clause"><strong>4.3.</strong> O pagamento da parcela devida à PARCEIRA será realizado pela ${nomeContratadaContrato} em até <strong>${prazoPgto} (${prazoPgto === 5 ? 'cinco' : String(prazoPgto)}) dias úteis</strong> após a confirmação do recebimento integral da comissão paga pelo CLIENTE.</p>
...
<p class="clause"><strong>5.1.</strong> As partes reconhecem que a prestação de serviços objeto deste contrato é uma atividade de meio, e não de resultado. A ${nomeContratadaContrato} e a PARCEIRA se comprometem a empregar seus melhores esforços e conhecimentos técnicos na busca pela obtenção do crédito para o CLIENTE, contudo, não garantem a sua efetiva contratação, que depende de critérios exclusivos da instituição financeira.</p>
<p class="clause"><strong>5.2.</strong> A não obtenção do crédito não gerará qualquer direito a indenização ou penalidade para qualquer das partes, tampouco obrigará a ${nomeContratadaContrato} ou a PARCEIRA a devolverem eventuais valores recebidos a título de taxas administrativas ou custos operacionais, caso aplicável e previamente acordado com o CLIENTE.</p>
<h2 class="section-title">CLÁUSULA SEXTA – DA AUSÊNCIA DE VÍNCULO EMPREGATÍCIO</h2>
<p class="clause"><strong>6.1.</strong> O presente contrato é de natureza estritamente cível e comercial, não gerando qualquer tipo de vínculo empregatício, societário ou de representação comercial entre a ${nomeContratadaContrato} e a PARCEIRA.</p>
...
`;
  ...
  async function gerarPdfComLayout(html: string, payload: any, filePath: string): Promise<void> {
    ...
    try {
      ...
      const brandingContaL = await getAccountBranding().catch(() => null);
      // Detectar contratada para escolher logo e cor; a Central de Personalização tem prioridade.
      ...
      const logoB64L    = brandingContaL ? (brandingContaL.logo_data_uri || '') : (isPermuPayL ? PERMUPAY_LOGO_B64 : DESTRAVA_LOGO_B64);
      const corBordaL   = brandingContaL?.cor_primaria || (isPermuPayL ? '#0066CC' : '#1B3A8C');
      const altLogoL    = brandingContaL?.nome_exibicao || (isPermuPayL ? 'PermuPay' : 'Destrava Crédito');
      const marcaHeaderL = logoB64L
        ? `<img src="${logoB64L}" alt="${escapeHtmlContrato(altLogoL)}"/>`
        : `<strong>${escapeHtmlContrato(altLogoL)}</strong>`;
      const headerTemplateL = `<style>* { margin: 0; padding: 0; box-sizing: border-box; } #hw { width: 100%; padding: 6px 22mm 8px; border-bottom: 2px solid ${corBordaL}; display: flex; align-items: center; justify-content: center; font-family: Arial, sans-serif; font-size: 12pt; color: ${corBordaL}; } img { height: 40px; max-width: 160px; object-fit: contain; display: block; }</style><div id="hw">${marcaHeaderL}</div>`;
      const emptyHeaderL    = '<style>* { margin: 0; padding: 0; }</style><div></div>';
      const rodapeConfiguradoL = brandingContaL ? [brandingContaL.rodape_linha_1, brandingContaL.rodape_linha_2, brandingContaL.rodape_linha_3].filter(Boolean).join('<br/>') : '';
      const rodapeFallbackContaL = brandingContaL
        ? [brandingContaL.nome_exibicao, [brandingContaL.email, brandingContaL.telefone || brandingContaL.whatsapp].filter(Boolean).join(' • '), [brandingContaL.endereco, brandingContaL.cidade, brandingContaL.uf, brandingContaL.cep].filter(Boolean).join(' • ')].filter(Boolean).map(escapeHtmlContrato).join('<br/>')
        : '<strong>BRASÍLIA - SEDE</strong><br/>St. D Norte QND 25 LOTE 40 - Taguatinga, Brasília - DF, 72120-250<br/><strong>GOIÂNIA - FILIAL</strong><br/>Avenida Afonso Pena, qd-25 Alt. 05, S/N sala-02 setor Goiânia 2 CEP: 74665555 Goiânia-GO';
      const footerTemplateL = `<style>* { margin: 0; padding: 0; box-sizing: border-box; } #fw { width: 100%; padding: 8px 22mm 6px; border-top: 1px solid #e2e8f0; text-align: center; font-family: Arial, sans-serif; font-size: 7.5pt; color: #64748b; line-height: 1.5; }</style><div id="fw">${rodapeConfiguradoL || rodapeFallbackContaL}</div>`;
  ...
  function gerarHtmlFichaClientePf(cliente: any, documentos: any[]): string {
    ...
    return gerarHtmlTimbrado(`
      ...
      <div style="margin-top:12px; font-size:10px; color:#94a3b8; text-align:center;">
        Gerado em ${new Date().toLocaleString('pt-BR')} — ${nomeMarca}
      </div>
...
async function registrarDocumentoContratoGerado(params: {
  ...
  app.post('/api/contratos/gerar', auth, uploadContratos.any(), async (req: Request, res: Response) => {
    try {
      ...
      const contratadaContaPadrao = await contratadaContaAtualNormalizada();
      const CONTRATADA = contratadaParaFormatoLegado(contratadaContaPadrao);
      let pdfPath: string;
      ...
      if (tipo_contrato === 'rating') {
        ...
        const contratadaRating = contratada_id ? await buscarPrestadorServicoAtivo(contratada_id) : await contratadaContaAtualNormalizada();
      ...
      if (tipo_contrato === 'assessoria_pf') {
        ...
        let contratadaPF: any = contratadaContaPadrao;
      ...
      let contratadaAssessoria: any = contratadaContaPadrao;
  ...
  function gerarHtmlRelatorioMensalAcompanhamento(payload: {
    ...
    marca?: string | null;
    accountBranding?: any | null;
    allowLegacyBrandOverride?: boolean;
  }): string {
    ...
    const brandingConta = payload.accountBranding || null;
    // Para a conta legada do Destrava, preserva a seleção explícita PermuPay já existente.
    // Para novas contas, a Central de Personalização é a identidade única dos relatórios.
    const usarMarcaLegadaExplicita = payload.allowLegacyBrandOverride === true && marcaRelatorio === "permupay";
    const logoRelatorio = !usarMarcaLegadaExplicita && brandingConta?.logo_data_uri
      ? brandingConta.logo_data_uri
      : marcaRelatorio === "permupay" ? PERMUPAY_LOGO_B64 : DESTRAVA_LOGO_B64;
    const nomePrestadora = !usarMarcaLegadaExplicita && brandingConta
      ? (brandingConta.nome_exibicao || brandingConta.nome_fantasia || brandingConta.razao_social || "Empresa")
      : marcaRelatorio === "permupay" ? "PermuPay" : "Destrava Crédito";
    const razaoPrestadora = !usarMarcaLegadaExplicita && brandingConta
      ? (brandingConta.razao_social || brandingConta.nome_exibicao || nomePrestadora)
      : nomePrestadora === "Destrava Crédito" ? "DESTRAVA CRÉDITO LTDA" : nomePrestadora.toUpperCase();
    const cnpjPrestadora = !usarMarcaLegadaExplicita && brandingConta?.cnpj
      ? `CNPJ ${brandingConta.cnpj}`
      : marcaRelatorio === "permupay" ? "" : "CNPJ 35.427.182/0001-66";
...
<body>
  ...
  <div class="assinaturas">
    <div class="assinatura">
      <strong>${esc(razaoPrestadora)}</strong><br/>
  ...
  async function responderRelatorioAcompanhamentoBancario(req: Request, res: Response) {
    try {
      ...
      const brandingRelatorioConta = await getAccountBranding().catch(() => null);
      const contaLegadaId = process.env.DEFAULT_CONTA_ID || "00000000-0000-4000-8000-000000000001";
      const html = gerarHtmlRelatorioMensalAcompanhamento({
        ...
        marca,
        accountBranding: brandingRelatorioConta,
        allowLegacyBrandOverride: String((req as any).user?.conta_id || "") === contaLegadaId,
      });
  ...
  function gerarHtmlRelatorioFinanceiro(payload: {
    ...
    const dataEmissao = new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' });
    const brandingConta = getCachedAccountBranding();
    const razaoPrestadora = brandingConta?.razao_social || brandingConta?.nome_exibicao || "DESTRAVA CRÉDITO LTDA";
    const documentoPrestadora = brandingConta?.cnpj ? `CNPJ ${brandingConta.cnpj}` : "CNPJ 35.427.182/0001-66";
    ...
    <div style="margin-top:18px;padding-top:8px;border-top:1px solid #d1d5db;font-size:7.5pt;color:#6b7280;display:flex;justify-content:space-between;">
      ...
      <span>${esc(razaoPrestadora)} &nbsp;|&nbsp; ${esc(documentoPrestadora)}</span>
    </div>
    `;
