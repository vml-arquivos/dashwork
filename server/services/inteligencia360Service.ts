...
export interface DocumentoIdentidade360 {
  codigo: "cartao_cnpj" | "qsa" | "enquadramento_tributario";
  nome: string;
  anexado: boolean;
  analisado: boolean;
  consistente: boolean;
...
export interface Inteligencia360Result {
  ...
  // Portão inicial: Cartão CNPJ, QSA e Enquadramento Tributário.
...
export function consolidarEtapaIdentidadeDocumental(params: {
  ...
  const situacaoAtiva = ["ativa", "ativo", "regular", "habilitada", "habilitado", "apta", "apto"].includes(situacao);
  ...
  return {
    ...
    avisos,
    diagnostico: apto
      ? "Os três documentos iniciais foram lidos e considerados consistentes. A empresa pode avançar para a conferência do Contrato Social/Alteração com os Atos da Junta Comercial."
      : "A etapa inicial ainda não foi concluída. Analise ou corrija somente os itens indicados antes de avançar.",
    proxima_etapa: "Contrato Social/Alteração e Atos da Junta Comercial, com validação por NIRE e data de registro.",
  };
}
...
export function calcularInteligencia360(params: {
  ...
  if (!etapa_identidade_documental.apto_para_avancar) {
    ...
    caminhoFinal = "Concluir primeiro a leitura e a validação dos três documentos de Identidade do CNPJ.";
    proximas_acoes = [
      "Analisar Cartão CNPJ, QSA e Enquadramento Tributário.",
