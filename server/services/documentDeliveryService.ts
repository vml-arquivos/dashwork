...
import { getAccountBranding } from "./accountBrandingService";
...
function montarMensagemPadrao(tipoDocumento: string, nomeDestinatario: string | null | undefined, marca: string, linkOpcional?: string): string {
  const rotulo = rotuloTipoDocumento(tipoDocumento);
  const saudacao = nomeDestinatario ? `Olá, ${nomeDestinatario}!` : "Olá!";
  const base = `${saudacao} Segue o(a) ${rotulo} da ${marca}.`;
  return linkOpcional ? `${base} ${linkOpcional}` : base;
}

async function marcaDaConta(): Promise<string> {
  const branding = await getAccountBranding().catch(() => null);
  return branding?.nome_exibicao || branding?.nome_fantasia || branding?.razao_social || process.env.APP_NAME || "Destrava Crédito";
}

async function enviarPorEmail(input: EnviarDocumentoInput): Promise<EnviarDocumentoResultado> {
  const apiKey = process.env.RESEND_API_KEY;
  const marca = await marcaDaConta();
  const remetente = process.env.RESEND_FROM_EMAIL || `${marca} <nao-responda@destravacredito.com>`;
  ...
  const assunto = input.assunto || `${marca} — ${rotuloTipoDocumento(input.tipoDocumento)}`;
  const mensagem = input.mensagem || montarMensagemPadrao(input.tipoDocumento, input.destinatario.nome, marca);
...
async function gerarLinkWhatsapp(pool: Pool, input: EnviarDocumentoInput): Promise<EnviarDocumentoResultado & { token?: string; tokenExpiraEm?: Date }> {
  ...
  const marca = await marcaDaConta();
  const mensagem = input.mensagem || montarMensagemPadrao(input.tipoDocumento, input.destinatario.nome, marca, linkPublico);
