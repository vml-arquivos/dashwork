...
function gerarParecerTecnico(
  ...
  parecer += `${scoreLabel}: ${scoreDestrava}/100. `;
...
function gerarResumoExecutivo(
  ...
  if (bloqueantes.length > 0) {
    return `${razao} necessita complementação documental antes da análise de crédito. ${scoreLabel}: ${scoreDestrava}/100. ${bloqueantes.length} pendência(s) crítica(s) identificada(s). Resolva as pendências para avançar com a proposta.`;
  }

  if (proposta.valorSugerido) {
    return `${razao} está apta para análise preliminar de crédito. ${scoreLabel}: ${scoreDestrava}/100. Capacidade estimada com base nos dados disponíveis: ${fmtBRL(proposta.valorSugerido)}. Proposta sujeita à análise bancária.`;
  }

  return `${razao} está cadastrada no sistema. ${scoreLabel}: ${scoreDestrava}/100. Dados insuficientes para estimativa completa. Complementar informações financeiras e documentação para avançar com a proposta.`;
}
...
export function calcularPropostaBancaria(input: PropostaBancariaInput): PropostaBancariaResult {
  ...
  const parecer = gerarParecerTecnico(empresa, scoreDestrava, nivelRisco, pendencias, capacidade, proposta, scoreLabel);
  const proximosPassos = gerarProximosPassos(pendencias, proposta);
  const resumo = gerarResumoExecutivo(empresa, scoreDestrava, pendencias, proposta, scoreLabel);
