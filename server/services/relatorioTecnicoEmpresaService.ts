...
function gerarResumoExecutivo(empresa: any, score: number, pendencias: PendenciaRelatorio[], analiseCredito: AnaliseCredito, marcaSistema: string, scoreLabel: string): string {
  const razao = safeStr(empresa?.razao_social);
  const cnpj = safeStr(empresa?.cnpj, "CNPJ não informado");
  const criticas = pendencias.filter(p => p.prioridade === "critica");
  const fat = safeNum(empresa?.faturamento_anual);

  let resumo = `A empresa ${razao} (${cnpj}) foi analisada com base nos dados disponíveis na plataforma ${marcaSistema}. `;
  resumo += `${scoreLabel}: ${score}/100. `;
  ...
  resumo += `Este relatório é de caráter técnico e consultivo, destinado a apoiar a análise interna da ${marcaSistema} e o relacionamento com parceiros financeiros. Proposta sujeita à análise bancária e critérios da instituição financeira parceira.`;
...
export function gerarRelatorioTecnico(input: RelatorioTecnicoInput): RelatorioTecnicoResult {
  ...
  const marcaSistema = safeStr(input?.marca_sistema, "Destrava Crédito");
  const scoreLabel = marcaSistema.toLowerCase().includes("destrava") ? "Score Destrava" : `Score ${marcaSistema}`;
  const responsavel = safeStr(input?.responsavel_nome, `Sistema ${marcaSistema}`);
  ...
  const analiseCredito: AnaliseCredito = {
    ...
    parecer: `Empresa ${safeStr(empresa?.razao_social)} apresenta-se ${statusProposta.toLowerCase()}. ${scoreLabel}: ${score}/100. Proposta sujeita à análise bancária e critérios da instituição financeira parceira.`,
    status_proposta: statusProposta,
  };

  const resumoExecutivo = gerarResumoExecutivo(empresa, score, pendencias, analiseCredito, marcaSistema, scoreLabel);
  ...
  return {
    ...
    observacoes_legais: `Este relatório tem caráter técnico e consultivo. As análises e estimativas apresentadas são baseadas nos dados disponíveis na plataforma ${marcaSistema} e não constituem garantia de aprovação de crédito, assessoria jurídica, contábil ou financeira formal. Proposta sujeita à análise bancária e critérios da instituição financeira parceira. ${marcaSistema} não se responsabiliza por decisões tomadas com base exclusiva neste relatório.`,
