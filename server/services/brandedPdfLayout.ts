...
import { getAccountBranding, type AccountBranding } from "./accountBrandingService";
...
function accountHeaderTemplate(b: AccountBranding): string {
  const content = b.logo_data_uri ? `<img src="${b.logo_data_uri}" alt="${escapeHtml(b.nome_exibicao)}"/>` : `<span>${escapeHtml(b.nome_exibicao)}</span>`;
  return `<style>*{margin:0;padding:0;box-sizing:border-box}#hw{width:100%;padding:6px 22mm 8px;border-bottom:2px solid ${b.cor_primaria};display:flex;align-items:center;justify-content:center;background:#fff;font-family:Arial,sans-serif;font-size:11pt;font-weight:700;color:${b.cor_primaria}}img{height:40px;max-width:160px;object-fit:contain;display:block}</style><div id="hw">${content}</div>`;
}
function accountFooterTemplate(b: AccountBranding): string {
 const lines=[b.rodape_linha_1,b.rodape_linha_2,b.rodape_linha_3].filter(Boolean);
 const fallback=[b.nome_exibicao,[b.email,b.telefone||b.whatsapp].filter(Boolean).join(' • '),[b.endereco,b.cidade,b.uf,b.cep].filter(Boolean).join(' • ')].filter(Boolean);
 return `<style>*{margin:0;padding:0;box-sizing:border-box}#fw{width:100%;padding:8px 22mm 6px;border-top:1px solid #e2e8f0;text-align:center;font-family:Arial,sans-serif;font-size:7.5pt;color:#64748b;line-height:1.5}</style><div id="fw">${(lines.length?lines:fallback).map(x=>escapeHtml(x)).join('<br/>')}</div>`;
}
...
export async function generateBrandedPdfBuffer(
  ...
  try {
    ...
    const account = await getAccountBranding().catch(() => null);
    const explicitLegacyBrand = normalizeBrand(options.brand);
    // Compatibilidade da conta Destrava original: orçamentos antigos podiam ser
    // emitidos explicitamente como PermuPay/Aragão. Preservamos essa escolha só
    // na conta legada. Nas novas contas, a Central de Personalização é a marca
    // única de todos os documentos gerados.
    const allowLegacyBrandOverride = account?.conta_id === LEGACY_DEFAULT_CONTA_ID
      && (explicitLegacyBrand === "permupay" || explicitLegacyBrand === "aragao");
    const header = account && !allowLegacyBrandOverride
      ? accountHeaderTemplate(account)
      : headerTemplate(options.brand);
    const footer = account && !allowLegacyBrandOverride
      ? accountFooterTemplate(account)
      : FOOTER_TEMPLATE;
